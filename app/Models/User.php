...
class User extends Model
{
    ...
    function studentID()
    {
        ...
        if ($query != null) {
            $no = substr($query['username'], 6, 10);
            // dd($no >= 9999);

            if ($no >= 9999) {
                $username = substr($hijri['year'], 2) . sprintf('%02s', $hijri['month']) . $reg . sprintf('%04s', 1);
            } else {
                $invc = $no + 1;
                $username = substr($hijri['year'], 2) . sprintf('%02s', $hijri['month']) . $reg . sprintf('%04s', $invc);
            }
        } else {
            $username = substr($hijri['year'], 2) . sprintf('%02s', $hijri['month']) . $reg . sprintf('%04s', 1);
        }
